try{
  ...
  console.error('LevelUp is NOT installed, please "npm install levelup" to use this module');
  throw e;
}
...
try{
  ...
  console.error(provider+' is NOT installed, please "npm install '+provider+'" to use this module');
  throw e;
}
*/
try{
  ...
  console.error('bson-objectid not found, falling back to default uuid');
  var uuid = function(){
    id = (new Date()).getTime();
...
Store.init = function(config){
  var location = config.location||'./data';
  var providerName = config.provider||'leveldown';
  try{
  ...
  var _db = levelup(location, {db: provider, valueEncoding: 'json'});
};

Store.prototype.get = function(_id, callback){
  var self = this;
  _db.get(self.collectionName+':'+_id, function(err, record){
    if(err){
...
Store.prototype.insert = function(record, callback){
  ...
  record._id = id;
  _db.put(self.collectionName+':'+id, record, function(err){
    if(err){
      return callback(err.toString());
    }
...
Store.prototype.update = function(_id, record, callback){
  var self = this;
  _db.get(self.collectionName+':'+_id, function(err, curr){
    if(err){
      return callback(err.toString());
    }
    if(record){
      ...
      record._id = _id;
      _db.put(self.collectionName+':'+_id, record, function(err){
        if(err){
          return callback(err.toString());
        }
        return callback(null, record);
...
Store.prototype.delete = function(id, callback){
  var self = this;
  _db.del(self.collectionName+':'+id, function(err){
    if(err){
